...
function generateRemark(index, port, address, cleanIPs, protocol, configType) {
    ...
    return `馃挦 ${index} - ${protocol}${type} - ${addressType} : ${port}`;
}
...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<body>
		<h1>POORIA Panel <span style="font-size: smaller;">${panelVersion}</span> 馃挦</h1>
		<div class="form-container">
            <form id="configForm">
                <h2>VLESS/TROJAN SETTINGS 鈿欙笍</h2>
				...
				<div class="form-control">
					<label for="proxyIP">馃搷 Proxy IP</label>
                ...
                <div class="form-control">
                    <label for="fragmentPackets">馃摝 Packets</label>
				...
				<div class="form-control">
                    <label for="warpPlusLicense">鉃� Warp+ License</label>
                ...
                <div class="form-control">
                    <label style="line-height: 1.5;">馃攷 Scan Endpoint</label>
                ...
                <div class="form-control">
					<label for="bestWarpInterval">馃攧 Best Interval</label>
					<input type="number" id="bestWarpInterval" name="bestWarpInterval" min="10" max="90" value="${bestWarpInterval}">
				</div>
                <h2>WARP PRO SETTINGS 鈿欙笍</h2>
                <div class="form-control">
					<label for="hiddifyNoiseMode">馃樀鈥嶐煉� Hiddify Mode</label>
                ...
                <div class="form-control">
					<label for="nikaNGNoiseMode">馃樀鈥嶐煉� NikaNG Mode</label>
                ...
                <div class="form-control">
					<label for="noiseCountMin">馃帤锔� Noise Count</label>
					<div style="display: grid; grid-template-columns: 1fr auto 1fr; align-items: baseline;">
                ...
                <div class="form-control">
					<label for="noiseSizeMin">馃搹 Noise Size</label>
					<div style="display: grid; grid-template-columns: 1fr auto 1fr; align-items: baseline;">
                ...
                <div class="form-control">
					<label for="noiseDelayMin">馃暈 Noise Delay</label>
					<div style="display: grid; grid-template-columns: 1fr auto 1fr; align-items: baseline;">
				...
				<div id="apply" class="form-control">
					<div style="grid-column: 2; width: 100%; display: inline-flex;">
						<input type="submit" id="applyButton" style="margin-right: 10px;" class="button disabled" value="APPLY SETTINGS 馃挜" form="configForm">
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}?app=hiddify#POORIA-Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}?app=hiddify#POORIA-Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
	...
	<script>
		...
		document.addEventListener('DOMContentLoaded', async () => {
            ...
            resetSettings.addEventListener('click', async () => {
                const confirmReset = confirm('鈿狅笍 Are you sure?');
                ...
                try {
                    ...
                    refreshBtn.innerHTML = '鈱� Loading...';
                    ...
                    if (response.ok) {
                        alert('鉁� Panel settings reset to default successfully! 馃槑');
                        window.location.reload(true);
                    } else {
                        const errorMessage = await response.text();
                        console.error(errorMessage, response.status);
                        alert('鈿狅笍 An error occured, Please try again!\\n鉀� ' + errorMessage);
                    }         
                } catch (error) {
                    console.error('Error:', error);
                }
        ...
        const getWarpConfigs = async () => {
            ...
            if (license !== warpPlusLicense) {
                alert('鈿狅笍 First APPLY SETTINGS and then update Warp configs!');
                return false;
            }
            ...
            try {
                ...
                refreshBtn.innerHTML = refreshButtonVal;
                if (response.ok) {
                    ${isWarpPlus} ? alert('鉁� Warp configs upgraded to PLUS successfully! 馃槑') : alert('鉁� Warp configs updated successfully! 馃槑');
                } else {
                    const errorMessage = await response.text();
                    console.error(errorMessage, response.status);
                    alert('鈿狅笍 An error occured, Please try again!\\n鉀� ' + errorMessage);
        ...
        const handlePortChange = (event) => {
            ...
            if (activeHttpsPortsNo === 0) {
                event.preventDefault();
                event.target.checked = !event.target.checked;
                alert("鉀� At least one TLS(https) port should be selected! 馃");
        ...
        const handleProtocolChange = (event) => {
            ...
            if (activeProtocols === 0) {
                event.preventDefault();
                event.target.checked = !event.target.checked;
                alert("鉀� At least one Protocol should be selected! 馃");
                activeProtocols = 1;
                return false;
            }
        }
        ...
        const applySettings = async (event, configForm) => {
            ...
            if (isVless && securityType === 'tls' && vlessPort !== '443') {
                alert('鉀� VLESS TLS port can be only 443 to be used as a proxy chain! 馃');               
                return false;
            }

            if (isCustomCdn && !(customCdnAddrs && customCdnHost && customCdnSni)) {
                alert('鉀� All "Custom" fields should be filled! 馃');               
                return false;
            }
            ...
            try {
                ...
                if (response.ok) {
                    alert('鉁� Parameters applied successfully 馃槑');
                    window.location.reload(true);
                } else {
                    const errorMessage = await response.text();
                    console.error(errorMessage, response.status);
        ...
        const resetPassword = async (event) => {
            ...
            try {
                ...
                if (response.ok) {
                    modal.style.display = "none";
                    document.body.style.overflow = "";
                    alert("鉁� Password changed successfully! 馃憤");
                    window.location.href = '/login';
    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `馃挦 Warp Pro ${index + 1} 馃嚠馃嚪` : `馃挦 Warp ${index + 1} 馃嚠馃嚪`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
        ...
        if (client === 'clash') {
            let clashOutbound = buildClashWarpOutbound(`馃挦 Warp ${index + 1} 馃嚠馃嚪`, warpIPv6, privateKey, publicKey, endpoint, reserved, '');
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (client === 'clash') {
                let clashOutbound = buildClashWarpOutbound(
                    i === 1 ? `warp-ir_${index + 1}` : `馃挦 WoW ${index + 1} 馃實`, 
                    warpIPv6, 
                    privateKey, 
                    publicKey, 
                    endpoint,
...
async function getFragmentConfigs(env, hostName) {
    ...
    bestFragment.remarks = '⚔POORIA F - Best Fragment POORIA';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `⚔POORIA - Warp Pro ${index + 1} ⚔` : `⚔POORIA - Warp ${index + 1} ⚔`,
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    config['proxy-groups'][0].proxies = isWarp
        ? ['馃挦 Warp Best Ping 馃殌', '馃挦 WoW Best Ping 馃殌', ...warpOutboundsRemarks, ...wowOutboundRemarks ]
        : ['馃挦 Best Ping 馃挜', ...outboundsRemarks ];

    config['proxy-groups'][1].proxies = isWarp ? warpOutboundsRemarks : outboundsRemarks;
    config['proxy-groups'][1].name = isWarp ? `馃挦 Warp Best Ping 馃殌`: `馃挦 Best Ping 馃挜`,

    isWarp && config["proxy-groups"].push({
        "name": "馃挦 WoW Best Ping 馃殌",
        "type": "url-test",
...
async function getSingboxConfig (env, hostName, client, isWarp, isFragment) {
    ...
    if (isWarp) {
        ...
        config.outbounds[0].outbounds = client === 'hiddify'
            ? ["馃挦 Warp Pro Best Ping 馃殌", "馃挦 WoW Pro Best Ping 馃殌"]
            : ["馃挦 Warp Best Ping 馃殌", "馃挦 WoW Best Ping 馃殌"];
        config.outbounds.splice(2, 0, structuredClone(config.outbounds[1]));
        config.outbounds[1].tag = client === 'hiddify' 
            ? "馃挦 Warp Pro Best Ping 馃殌"
            : "馃挦 Warp Best Ping 馃殌";
        config.outbounds[2].tag = client === 'hiddify'
            ? "馃挦 WoW Pro Best Ping 馃殌"
            : "馃挦 WoW Best Ping 馃殌";
...
async function getNormalConfigs(env, hostName, client) {
    ...
    if (outProxy) {
        let chainRemark = `#${encodeURIComponent('馃挦 Chain proxy 馃敆')}`;
